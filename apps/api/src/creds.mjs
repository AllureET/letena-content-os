...
export const CRED_REGISTRY = [
  ...
  { key: 'FAL_API_KEY', label: 'fal API key', group: 'Production services', secret: true,
    hint: 'Lip-synced talking head: the presenter is animated FROM the shot\'s narration, so her mouth matches the words. Get it from fal.ai. Without it, shots set to talking head cannot generate; Runway shots are unaffected.' },
  { key: 'FAL_TALKING_HEAD_MODEL', label: 'Talking head model', group: 'Production services', secret: false,
    hint: 'Blank uses veed/fabric-1.0, which takes a still plus audio and returns the person speaking it, up to 300 seconds. Change this only to move to another fal model with the same image_url / audio_url inputs.' },
  ...
  { key: 'GEMINI_API_KEY', label: 'Gemini API key', group: 'Production services', secret: true,
    hint: 'Image generation, reference-sheet splitting, continuity QC, and Amharic transcription.' },
  { key: 'GEMINI_IMAGE_MODEL', label: 'Gemini image model', group: 'Production services', secret: false,
    hint: 'Blank uses gemini-2.5-flash-image. Change this only when Google retires the model; the error message they return names the replacement.' },
